...
const doctorRender = (arr, list = doctorList) => {
    ...
    <a href="../pages/singleClinic.html" data-doctor-id="${
			el?._id
		}" class="card_box doctor_id">
    <div class="card_top doctor_id " data-doctor-id="${el?._id}">
<img class="card_top_img doctor_id" data-doctor-id="${
			el?._id
		}" src="http://16.171.196.156/${el?.image}" alt="">
    </div>
...
const Deliveries = () => {
	fetch(`http://16.171.196.156/api/doctors`, {
		method: 'GET',
	})
		.then((res) => {
			return res.json();
